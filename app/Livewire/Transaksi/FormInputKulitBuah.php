...
class FormInputKulitBuah extends Component
{
    use WithFileUploads;
    public $periode;
    public $unit;
    public $dataKulitbuah;
    public $digunakan;
    public $dikirim;
    ...
    public $id_data_kulitbuah;
    public $photo;
    public $isAllowed = false;
    public $id_produksi;
    public $tbs_olah;
    public $produksi_kulitbuah;
    public function mount($periode, $unit)
    {
        $this->periode = $periode;
        $this->unit = $unit;
        list($bulan, $tahun) = explode('/', $periode);
        $dataKulitbuah = MasterUnitN1::query()
            ->unitWithKulitbuah($unit, $tahun, $bulan)
            ->first();
        if ($dataKulitbuah) {
            if ($dataKulitbuah->id_kulitbuah) {
                $this->digunakan = $dataKulitbuah->digunakan;
                $this->dikirim = $dataKulitbuah->dikirim;
                $this->volume_keperluan_lain = $dataKulitbuah->volume_keperluan_lain;
                $this->keterangan_keperluan_lain = $dataKulitbuah->keterangan_keperluan_lain;
                $this->dijual = $dataKulitbuah->dijual;
                $this->harga_jual_rata_rata = number_format($dataKulitbuah->harga_jual_rata_rata, 2, ',', '.');
                $this->id_data_kulitbuah = $dataKulitbuah->id_kulitbuah;
            } else {
                $this->cleanData();
            }
            $this->produksi_kulitbuah = $dataKulitbuah->produksi_kulitbuah;
            $this->tbs_olah = $dataKulitbuah->tbs_olah;
            $this->id_produksi = $dataKulitbuah->id_produksi;
            $this->isAllowed = true;
        }
        $this->dataKulitbuah = $dataKulitbuah;
    }
    public function render()
    {
        ...
        return view('livewire.transaksi.form-input-kulit-buah', compact('isPeriodeOpen'));
    }
    ...
    public function setData($periode, $unit)
    {
        ...
        $dataKulitbuah = MasterUnitN1::query()
            ...
            ->first();
        if ($dataKulitbuah) {
            if ($dataKulitbuah->id_kulitbuahe) {
                $this->digunakan = $dataKulitbuah->digunakan;
                $this->dikirim = $dataKulitbuah->dikirim;
                $this->volume_keperluan_lain = $dataKulitbuah->volume_keperluan_lain;
                $this->keterangan_keperluan_lain = $dataKulitbuah->keterangan_keperluan_lain;
    ...
    public function submit()
    {
        try {
            ...
            DB::transaction(function () {
                ...
                TransaksiKulitbuah::create($data);
                TransaksiEvidence::create([
                    'uuid' => Str::uuid(),
                    'kode_unit' => $this->unit,
                    'id_transaksi' => $data['uuid'],
                    'nama_file' => $this->photo->store('kulitbuah', 'public'),
                    'kategori' => 'kulitbuah',
    ...
    {
        list($bulan, $tahun) = explode('/', $this->periode);
        $data = TransaksiKulitbuah::where('uuid', $this->id_data_kulitbuah)->first();
        try {
            $this->digunakan = str_replace('.', '', $this->digunakan);
            $this->dikirim = str_replace('.', '', $this->dikirim);
            $this->volume_keperluan_lain = str_replace('.', '', $this->volume_keperluan_lain);
