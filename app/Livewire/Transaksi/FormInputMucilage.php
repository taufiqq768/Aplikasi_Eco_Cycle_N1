...
class FormInputMucilage extends Component
{
    use WithFileUploads;
    public $periode;
    public $unit;
    ...
    public $id_data_mucilage;
    public $photo;
    public $isAllowed = false;
    public $id_produksi;
    public $tbs_olah;
    public $produksi_mucilage;
    public function mount($periode, $unit)
    {
        $this->periode = $periode;
        $this->unit = $unit;
        list($bulan, $tahun) = explode('/', $periode);
        $dataMucilage = MasterUnitN1::query()
            ->unitWithMucilage($unit, $tahun, $bulan)
            ->first();
        if ($dataMucilage) {
            if ($dataMucilage->id_mucilage) {
                $this->digunakan = $dataMucilage->digunakan;
                $this->dikirim = $dataMucilage->dikirim;
                $this->volume_keperluan_lain = $dataMucilage->volume_keperluan_lain;
                $this->keterangan_keperluan_lain = $dataMucilage->keterangan_keperluan_lain;
                $this->dijual = $dataMucilage->dijual;
                $this->harga_jual_rata_rata = number_format($dataMucilage->harga_jual_rata_rata, 2, ',', '.');
                $this->id_data_mucilage = $dataMucilage->id_mucilage;
            } else {
                $this->cleanData();
            }
            $this->produksi_mucilage = $dataMucilage->produksi_mucilage;
            $this->tbs_olah = $dataMucilage->tbs_olah;
            $this->id_produksi = $dataMucilage->id_produksi;
            $this->isAllowed = true;
        }
        $this->dataMucilage = $dataMucilage;
    }
    public function render()
    {
        ...
        return view('livewire.transaksi.form-input-mucilage', compact('isPeriodeOpen'));
    }
    ...
    public function submit()
    {
        try {
            ...
            DB::transaction(function () {
                ...
                TransaksiMucilage::create($data);
                TransaksiEvidence::create([
                    'uuid' => Str::uuid(),
                    'kode_unit' => $this->unit,
                    'id_transaksi' => $data['uuid'],
                    'nama_file' => $this->photo->store('mucilage', 'public'),
                    'kategori' => 'mucilage',
    ...
    {
        list($bulan, $tahun) = explode('/', $this->periode);
        $data = TransaksiMucilage::where('uuid', $this->id_data_mucilage)->first();
        try {
            $this->digunakan = str_replace('.', '', $this->digunakan);
            $this->dikirim = str_replace('.', '', $this->dikirim);
            $this->volume_keperluan_lain = str_replace('.', '', $this->volume_keperluan_lain);
