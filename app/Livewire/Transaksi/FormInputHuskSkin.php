...
class FormInputHuskskin extends Component
{
    use WithFileUploads;
    public $periode;
    public $unit;
    ...
    public $id_data_huskskin;
    public $photo;
    public $isAllowed = false;
    public $id_produksi;
    public $tbs_olah;
    public $produksi_huskskin;
    public function mount($periode, $unit)
    {
        $this->periode = $periode;
        $this->unit = $unit;
        list($bulan, $tahun) = explode('/', $periode);
        $dataHuskskin = MasterUnitN1::query()
            ->unitWithHuskskin($unit, $tahun, $bulan)
            ->first();
        if ($dataHuskskin) {
            if ($dataHuskskin->id_huskskin) {
                $this->digunakan = $dataHuskskin->digunakan;
                $this->dikirim = $dataHuskskin->dikirim;
                $this->volume_keperluan_lain = $dataHuskskin->volume_keperluan_lain;
                $this->keterangan_keperluan_lain = $dataHuskskin->keterangan_keperluan_lain;
                $this->dijual = $dataHuskskin->dijual;
                $this->harga_jual_rata_rata = number_format($dataHuskskin->harga_jual_rata_rata, 2, ',', '.');
                $this->id_data_huskskin = $dataHuskskin->id_huskskin;
            } else {
                $this->cleanData();
            }
            $this->produksi_huskskin = $dataHuskskin->produksi_huskskin;
            $this->tbs_olah = $dataHuskskin->tbs_olah;
            $this->id_produksi = $dataHuskskin->id_produksi;
            $this->isAllowed = true;
        }
        $this->dataHuskskin = $dataHuskskin;
    }
    public function render()
    {
        ...
        return view('livewire.transaksi.form-input-husk-skin', compact('isPeriodeOpen'));
    }
    ...
    public function submit()
    {
        try {
            ...
            DB::transaction(function () {
                ...
                TransaksiHuskskin::create($data);
                TransaksiEvidence::create([
                    'uuid' => Str::uuid(),
                    'kode_unit' => $this->unit,
                    'id_transaksi' => $data['uuid'],
                    'nama_file' => $this->photo->store('huskskin', 'public'),
                    'kategori' => 'huskskin',
    ...
    {
        list($bulan, $tahun) = explode('/', $this->periode);
        $data = TransaksiHuskskin::where('uuid', $this->id_data_huskskin)->first();
        try {
            $this->digunakan = str_replace('.', '', $this->digunakan);
            $this->dikirim = str_replace('.', '', $this->dikirim);
            $this->volume_keperluan_lain = str_replace('.', '', $this->volume_keperluan_lain);
