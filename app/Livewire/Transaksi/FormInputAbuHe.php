...
class FormInputAbuHe extends Component
{
    use WithFileUploads;
    public $periode;
    public $unit;
    ...
    public $diterima;
    public $id_data_abuhe;
    public $photo;
    public $isAllowed = false;
    public $id_produksi;
    public $tbs_olah;
    public $produksi_abuhe;
    public function mount($periode, $unit)
    {
        $this->periode = $periode;
        $this->unit = $unit;
        list($bulan, $tahun) = explode('/', $periode);
        $dataAbuhe = MasterUnitN1::query()
            ->unitWithAbuhe($unit, $tahun, $bulan)
            ->first();
        if ($dataAbuhe) {
            if ($dataAbuhe->id_abuhe) {
                $this->digunakan = $dataAbuhe->digunakan;
                $this->dikirim = $dataAbuhe->dikirim;
                $this->volume_keperluan_lain = $dataAbuhe->volume_keperluan_lain;
                $this->keterangan_keperluan_lain = $dataAbuhe->keterangan_keperluan_lain;
                $this->dijual = $dataAbuhe->dijual;
                $this->harga_jual_rata_rata = number_format($dataAbuhe->harga_jual_rata_rata, 2, ',', '.');
                $this->diterima = $dataAbuhe->diterima;
                $this->id_data_abuhe = $dataAbuhe->id_abuhe;
            } else {
                $this->cleanData();
            }
            $this->produksi_abuhe = $dataAbuhe->produksi_abuhe;
            $this->tbs_olah = $dataAbuhe->tbs_olah;
            $this->id_produksi = $dataAbuhe->id_produksi;
            $this->isAllowed = true;
        }
        $this->dataAbuhe = $dataAbuhe;
    }
    public function render()
    {
        ...
        return view('livewire.transaksi.form-input-abu-he', compact('isPeriodeOpen'));
    }
    ...
    public function setData($periode, $unit)
    {
        ...
        if ($dataAbuhe) {
            if ($dataAbuhe->id_abuhe) {
                $this->digunakan = $dataAbuhe->digunakan;
                $this->dibkirim = $dataAbuhe->dikirim;
                $this->volume_keperluan_lain = $dataAbuhe->volume_keperluan_lain;
                $this->keterangan_keperluan_lain = $dataAbuhe->keterangan_keperluan_lain;
                $this->dijual = $dataAbuhe->dijual;
                $this->harga_jual_rata_rata = number_format($dataAbuhe->harga_jual_rata_rata, 2, ',', '.');
    ...
    public function submit()
    {
        try {
            ...
            DB::transaction(function () {
                ...
                TransaksiAbuhe::create($data);
                TransaksiEvidence::create([
                    'uuid' => Str::uuid(),
                    'kode_unit' => $this->unit,
                    'id_transaksi' => $data['uuid'],
                    'nama_file' => $this->photo->store('abuhe', 'public'),
                    'kategori' => 'abuhe',
    ...
    {
        list($bulan, $tahun) = explode('/', $this->periode);
        $data = TransaksiAbuhe::where('uuid', $this->id_data_abuhe)->first();
        try {
            $this->digunakan = str_replace('.', '', $this->digunakan);
            $this->dikirim = str_replace('.', '', $this->dikirim);
            $this->volume_keperluan_lain = str_replace('.', '', $this->volume_keperluan_lain);
