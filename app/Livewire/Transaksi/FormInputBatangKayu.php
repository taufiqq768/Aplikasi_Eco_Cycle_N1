...
class FormInputBatangKayu extends Component
{
    use WithFileUploads;
    public $periode;
    public $unit;
    public $dataBatangkayu;
    public $digunakan;
    public $dikirim;
    ...
    public $id_data_batangkayu;
    public $photo;
    public $isAllowed = false;
    public $id_produksi;
    public $tbs_olah;
    public $produksi_batangkayu;
    public function mount($periode, $unit)
    {
        $this->periode = $periode;
        $this->unit = $unit;
        list($bulan, $tahun) = explode('/', $periode);
        $dataBatangkayu = MasterUnitN1::query()
            ->unitWithBatangkayu($unit, $tahun, $bulan)
            ->first();
        if ($dataBatangkayu) {
            if ($dataBatangkayu->id_batangkayu) {
                $this->digunakan = $dataBatangkayu->digunakan;
                $this->dikirim = $dataBatangkayu->dikirim;
                $this->volume_keperluan_lain = $dataBatangkayu->volume_keperluan_lain;
                $this->keterangan_keperluan_lain = $dataBatangkayu->keterangan_keperluan_lain;
                $this->dijual = $dataBatangkayu->dijual;
                $this->harga_jual_rata_rata = number_format($dataBatangkayu->harga_jual_rata_rata, 2, ',', '.');
                $this->id_data_batangkayu = $$dataBatangkayu->id_batangkayu;
            } else {
                $this->cleanData();
            }
            $this->produksi_batangkayu = $dataBatangkayu->produksi_batangkayu;
            $this->tbs_olah = $dataBatangkayu->tbs_olah;
            $this->id_produksi = $dataBatangkayu->id_produksi;
            $this->isAllowed = true;
        }
        $this->dataBatangkayu = $dataBatangkayu;
    }
    public function render()
    {
        ...
        return view('livewire.transaksi.form-input-batang-kayu', compact('isPeriodeOpen'));
    }

    #[On('setdataBatangkayu')]
    ...
    public function submit()
    {
        try {
            ...
            DB::transaction(function () {
                ...
                TransaksiBatangkayu::create($data);
                TransaksiEvidence::create([
                    'uuid' => Str::uuid(),
                    'kode_unit' => $this->unit,
                    'id_transaksi' => $data['uuid'],
                    'nama_file' => $this->photo->store('batangkayu', 'public'),
                    'kategori' => 'batangkayu',
    ...
    {
        list($bulan, $tahun) = explode('/', $this->periode);
        $data = TransaksiBatangkayu::where('uuid', $this->id_data_batangkayu)->first();
        try {
            $this->digunakan = str_replace('.', '', $this->digunakan);
            $this->dikirim = str_replace('.', '', $this->dikirim);
            $this->volume_keperluan_lain = str_replace('.', '', $this->volume_keperluan_lain);
