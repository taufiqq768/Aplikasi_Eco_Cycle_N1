...
class FormInputRubberTrap extends Component
{
    use WithFileUploads;
    public $periode;
    public $unit;
    public $dataRubbertrap;
    public $digunakan;
    public $dikirim;
    ...
    public $id_data_rubbertrap;
    public $photo;
    public $isAllowed = false;
    public $id_produksi;
    public $tbs_olah;
    public $produksi_rubbertrap;
    public function mount($periode, $unit)
    {
        $this->periode = $periode;
        $this->unit = $unit;
        list($bulan, $tahun) = explode('/', $periode);
        $dataRubbertrap = MasterUnitN1::query()
            ->unitWithRubbertrap($unit, $tahun, $bulan)
            ->first();
        if ($dataRubbertrap) {
            if ($dataRubbertrap->id_rubbertrap) {
                $this->digunakan = $dataRubbertrap->digunakan;
                $this->dikirim = $dataRubbertrap->dikirim;
                $this->volume_keperluan_lain = $dataRubbertrap->volume_keperluan_lain;
                $this->keterangan_keperluan_lain = $dataRubbertrap->keterangan_keperluan_lain;
                $this->dijual = $dataRubbertrap->dijual;
                $this->harga_jual_rata_rata = number_format($dataRubbertrap->harga_jual_rata_rata, 2, ',', '.');
                $this->id_data_rubbertrap = $dataRubbertrap->id_rubbertrap;
            } else {
                $this->cleanData();
            }
            $this->produksi_rubbertrap = $dataRubbertrap->produksi_rubbertrap;
            $this->tbs_olah = $dataRubbertrap->tbs_olah;
            $this->id_produksi = $dataRubbertrap->id_produksi;
            $this->isAllowed = true;
        }
        $this->dataRubbertrap = $dataRubbertrap;
    }
    public function render()
    {
        ...
        return view('livewire.transaksi.form-input-rubber-trap', compact('isPeriodeOpen'));
    }

    #[On('setdataRubbertrap')]
    ...
    public function submit()
    {
        try {
            ...
            DB::transaction(function () {
                ...
                TransaksiRubbertrap::create($data);
                TransaksiEvidence::create([
                    'uuid' => Str::uuid(),
                    'kode_unit' => $this->unit,
                    'id_transaksi' => $data['uuid'],
                    'nama_file' => $this->photo->store('rubbertrap', 'public'),
                    'kategori' => 'rubbertrap',
    ...
    {
        list($bulan, $tahun) = explode('/', $this->periode);
        $data = TransaksiRubbertrap::where('uuid', $this->id_data_rubbertrap)->first();
        try {
            $this->digunakan = str_replace('.', '', $this->digunakan);
            $this->dikirim = str_replace('.', '', $this->dikirim);
            $this->volume_keperluan_lain = str_replace('.', '', $this->volume_keperluan_lain);
            ...
            DB::transaction(function () use ($data, $changed, $dbData, $keterangan, $input) {
                ...
                if ($this->photo) {
                    ...
                    TransaksiEvidence::create([
                        ...
                        'kode_unit' => $this->unit,
                        'id_transaksi' => $data->uuid,
                        'nama_file' => $this->photo->store('brubbertrap', 'public'),
                        'kategori' => 'rubbertrap',
                        'upload_by' => auth()->user()->nik_sap
                    ]);
                    LogTransaksi::create([
