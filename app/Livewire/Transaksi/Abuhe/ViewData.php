...
namespace App\Livewire\Transaksi\Abuhe;
...
class ViewData extends Component
{
    public function render()
    {
        $tahun = date('Y');
        $bulan = date('m');
        return view('livewire.transaksi.abuhe.view-data', compact('tahun', 'bulan'));
    }

    public function getAbuhe($bulan, $tahun)
    {
        
        try {
            $dataAbuhe = MasterUnit::query()
                ->withAbuhe($bulan, $tahun)
                ->get();
            $totalData = $dataAbuhe->count();
    ...
    public function getChartAbuheSd(StoredProcedure_N1 $storedProcedure_N1, $bulan, $tahun)
    {
        ...
        $stokAwalTahun = $data->pluck('stok_abu_he_awal_tahun');
        $stokSaatIni = $data->pluck('sisa_abu_he');
        $pendapatan = $data->pluck('pendapatan_abu_he');
        $dijual = $data->pluck('penjualan_abu_he');
        $produksi = $data->pluck('produksi_abu_he');
        $diterima = $data->pluck('penerimaan_abu_he');
        $digunakan = $data->pluck('abu_he_digunakan');
