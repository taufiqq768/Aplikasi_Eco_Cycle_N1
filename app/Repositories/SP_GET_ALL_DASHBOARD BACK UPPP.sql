...
BEGIN
    SELECT
        ...
        (
            SELECT 
                IFNULL(SUM(ts.stok_cangkang), 0)
            FROM 
                t_stok ts
            JOIN 
                m_unit mu ON ts.kode_unit = mu.kode
            WHERE 
                mu.kode_region = m_region.kode
                AND ts.tahun = tahun

        ) AS stok_cangkang_awal_tahun,
        
        (
            SELECT 
                IFNULL(SUM(ts.stok_fiber), 0)
            FROM 
                t_stok ts
            JOIN 
                m_unit mu ON ts.kode_unit = mu.kode
            WHERE 
                mu.kode_region = m_region.kode
                AND ts.tahun = tahun
        ) AS stok_fiber_awal_tahun,        

        (
            SELECT 
                IFNULL(SUM(ts.stok_tankos), 0)
            FROM 
                t_stok ts
            JOIN 
                m_unit mu ON ts.kode_unit = mu.kode
            WHERE 
                mu.kode_region = m_region.kode
                AND ts.tahun = tahun
        ) AS stok_tankos_awal_tahun,
        
        (
            SELECT 
                IFNULL(SUM(ts.stok_abu_janjang), 0)
            FROM 
                t_stok ts
            JOIN 
                m_unit mu ON ts.kode_unit = mu.kode
            WHERE 
                mu.kode_region = m_region.kode
                AND ts.tahun = tahun
        ) AS stok_abu_janjang_awal_tahun,
        
        (
            SELECT 
                IFNULL(SUM(ts.stok_solid), 0)
            FROM 
                t_stok ts
            JOIN 
                m_unit mu ON ts.kode_unit = mu.kode
            WHERE 
                mu.kode_region = m_region.kode
                AND ts.tahun = tahun
        ) AS stok_solid_awal_tahun,
        
        (
            SELECT 
                IFNULL(SUM(ts.stok_pome_oil), 0)
            FROM 
                t_stok ts
            JOIN 
                m_unit mu ON ts.kode_unit = mu.kode
            WHERE 
                mu.kode_region = m_region.kode
                AND ts.tahun = tahun
        ) AS stok_pome_oil_awal_tahun,
        
        (
            SELECT 
                IFNULL(SUM(ts.stok_pkm), 0)
            FROM 
                t_stok ts
            JOIN 
                m_unit mu ON ts.kode_unit = mu.kode
       ...
       -- Sisa stok
		...
		), 0) + t_stok.stok_cangkang AS sisa_cangkang,
		...
		) + t_stok.stok_tankos, 0)  AS sisa_tankos,
		...
		) + t_stok.stok_abu_janjang, 0) AS sisa_abu_janjang,
		...
		)  + t_stok.stok_solid, 0) AS sisa_solid,
		
		t_stok.stok_pome_oil + IFNULL(
		SUM(
