...
class StoredProcedure_N1
{
    ...
    {
        return DB::select('CALL SP_GET_ALL_DASHBOARD_N1(?, ?)', [$tahun, $bulan]);
    }

    public function getDashboardDataBi_N1($bulan, $tahun)
    {
        return DB::select('CALL SP_GET_ALL_DASHBOARD_BI_N1(?, ?)', [$tahun, $bulan]);
    }

    public function getUnitAllData_N1($bulan, $tahun, $isBi = NULL)
    {
        return DB::select('CALL SP_GET_ALL_UNIT_DATA_N1(?, ?, ?)', [$tahun, $bulan, $isBi]);
    }
    ...
    {
        $data = DB::select('CALL SP_GET_ALL_UNIT_DATA_W_FORMAT_N1(?, ?, ?, ?)', [$tahun, $bulan, $isBi, $tipe]);
        return $data;
    }

    public function getUnitAllDataWithFormatRegion_N1($tahun, $bulan, $isBi, $tipe, $region)
    {
        return DB::select('CALL SP_GET_ALL_UNIT_DATA_W_FORMAT_REGION_N1(?, ?, ?, ?, ?)', [$tahun, $bulan, $isBi, $tipe, $region]);
    }

    public function getUnitAllDataWithFormatUnit_N1($tahun, $bulan, $isBi, $tipe, $unit)
    {
        return DB::select('CALL SP_GET_ALL_UNIT_DATA_W_FORMAT_UNIT_N1(?, ?, ?, ?, ?)', [$tahun, $bulan, $isBi, $tipe, $unit]);
    }
