    ...
    @if ($isAllowed)
        <form wire:submit='submit' class="form" enctype="multipart/form-data" wire:key='{{ rand() }}'>
            <div class="row mt-4">
                ...
                <div class="col-6 mt-3">
                    <label for="produksi_huskskin">Produksi Husk Skin (Kg)</label>
                    <input type="text" name="produksi_huskskin" id="produksi_huskskin"
                        class="form-control form-control-lg" value="@thousands($produksi_huskskin)" readonly>
                </div>
                ...
                @else
                    @if ($id_data_huskskin)
                        <div class="d-flex justify-content-start mt-2">
                            <img src="{{ asset('storage/' . $dataHuskskin->nama_file) }}" alt="Preview Gambar"
                                style="max-height: 300px; max-width: 300px;">
                        </div>
                    @endif
                @endif
