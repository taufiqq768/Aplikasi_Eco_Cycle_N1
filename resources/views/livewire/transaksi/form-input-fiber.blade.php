    ...
    @if ($isAllowed)
        <form wire:submit='submit' class="form" enctype="multipart/form-data" wire:key='{{ rand() }}'>
            <div class="row mt-4">
                ...
                <div class="col-6 mt-3" x-data="{ value: @entangle('produksi_fiber') }">
                    <label for="produksi_fiber">Jumlah Produksi Fiber (Kg)</label>
                    <input type="text" name="produksi_fiber" id="produksi_fiber" value="@thousands($produksi_fiber)"
                ...
                @else
                    @if ($id_data_fiber)
                        <div class="d-flex justify-content-start mt-2">
                            <img src="{{ asset('storage/' . $dataFiber->nama_file) }}" alt="Preview Gambar"
                                style="max-height: 300px; max-width: 300px;">
                        </div>
                    @endif
                @endif
