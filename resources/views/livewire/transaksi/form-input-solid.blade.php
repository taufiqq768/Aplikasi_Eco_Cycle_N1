    ...
    @if ($isAllowed)
        <form wire:submit='submit' class="form" enctype="multipart/form-data" wire:key='{{ rand() }}'>
            <div class="row mt-4">
                ...
                <div class="col-6 mt-3">
                    <label for="produksi_solid">Produksi Solid (Kg)</label>
                    <input type="text" name="produksi_solid" id="produksi_solid" class="form-control form-control-lg"
                        value="@thousands($produksi_solid)" readonly>
                </div>
                ...
                @else
                    @if ($id_data_solid)
                        <div class="d-flex justify-content-start mt-2">
                            <img src="{{ asset('storage/' . $dataSolid->nama_file) }}" alt="Preview Gambar"
                                style="max-height: 300px; max-width: 300px;">
                        </div>
                    @endif
                @endif
