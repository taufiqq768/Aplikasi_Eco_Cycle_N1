<div>
    ...
    @if ($isAllowed)
        <form wire:submit='submit' class="form" enctype="multipart/form-data" wire:key='{{ rand() }}'>
            <div class="row mt-4">
                ...
                <div class="col-6 mt-3">
                    <label for="produksi_pkm">Produksi PKM (Kg)</label>
                    <input type="text" name="produksi_pkm" id="produksi_pkm" class="form-control form-control-lg"
                        value="@thousands($produksi_pkm)" readonly>
                </div>
                <div class="col-6 mt-3" x-data="{ value: @entangle('inti_diolah') }">
                    <label for="inti_diolah">Inti Diolah (Kg)</label>
                    <input type="text" name="inti_diolah" id="inti_diolah" class="form-control form-control-lg"
                        wire:model='inti_diolah' x-mask:dynamic="$money($input, ',')">
                </div>
                ...
                @else
                    @if ($id_data_pkm)
                        <div class="d-flex justify-content-start mt-2">
                            <img src="{{ asset('storage/' . $dataPkm->nama_file) }}" alt="Preview Gambar"
                                style="max-height: 300px; max-width: 300px;">
                        </div>
                    @endif
                @endif
    ...
    @else
        <div class="alert alert-danger" role="alert">
            Data Produksi Tidak Tersedia / Unit Bukan PPIS
        </div>
    @endif
</div>
