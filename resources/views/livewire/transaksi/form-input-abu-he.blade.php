    ...
    @if ($isAllowed)
        <form wire:submit='submit' class="form" enctype="multipart/form-data" wire:key='{{ rand() }}'>
            <div class="row mt-4">
                ...
                <div class="col-6 mt-3">
                    <label for="produksi_abuhe">Produksi Abu HE (Kg)</label>
                    <input type="text" name="produksi_abuhe" id="produksi_abuhe"
                        class="form-control form-control-lg" value="@thousands($produksi_abuhe)" readonly>
                </div>
                ...
                @else
                    @if ($id_data_abuhe)
                        <div class="d-flex justify-content-start mt-2">
                            <img src="{{ asset('storage/' . $dataAbuhe->nama_file) }}" alt="Preview Gambar"
                                style="max-height: 300px; max-width: 300px;">
                        </div>
                    @endif
                @endif
